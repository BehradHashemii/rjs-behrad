...
export async function getArticlesFromFirestore() {
  try {
    ...
    console.warn("Firestore getArticles warning, falling back to local dataset:", error);
...
export async function getPortfoliosFromFirestore() {
  try {
    ...
    console.warn("Firestore getPortfolios warning, falling back to local dataset:", error);
...
export async function addMessageToTicketInFirestore(ticketId, messageText, sender = "user") {
  try {
    ...
    if (snapshot.exists()) {
      ...
      await updateDoc(ticketDoc, { messages: newMessages, updatedAt: new Date().toISOString() });
      return newMessages;
    }
  } catch (error) {
